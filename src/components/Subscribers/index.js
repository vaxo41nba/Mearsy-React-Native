...
const SubscribersAccount = ({
  ...
  useEffect(() => {
    const findIfSubscribed = loggedUserInfo.readers.filter((i) =>
      i.includes(user.userId));
    findIfSubscribed.length > 0
      ? setSubscribeStatus(false)
      : setSubscribeStatus(true);

    const findIfInBlacklist = loggedUserInfo.blackListOfUsers.filter((i) =>
      i.inclodes(user.userId));
    findIfInBlacklist.length > 0
    ...
    <View style={styles.mainContainer}>
      ...
      <View style={styles.buttonsContainer}>
        ...
        {blacklist ? (
          <>
            {blacklistStatus ? (
              <TouchableOpacity style={styles.removeFromBlacklistButton}>
                <Text style={styles.removeFromBlacklistText}>
...
export default SubscribersAccount;
